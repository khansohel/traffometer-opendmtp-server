...
public class StringTools
{
    ...
    public static StringBuffer formatHexString(byte b[], int maxRcdLen, StringBuffer sb)
    {
        ...
        for (int i = 1; i < maxRcdLen;) {
            for (int j = i; (i < maxRcdLen) & ((i - j) < 4); i++) { sb.append("-- "); }
            if (i < maxRcdLen) { sb.append("++ "); i++; }
            for (int j = i; (i < maxRcdLen) & ((i - j) < 4); i++) { sb.append("-- "); }
            if (i < maxRcdLen) { sb.append(format(i,"00 ")); i++; }
        }
        sb.append("\n");
    ...
    public static Map parseProperties(String props, Map properties)
    {
        ...
        for (;a < argsLen;) {
            ...
            while ((a < argsLen) && !Character.isWhitespace(args[a])) { a++; } // trailing junk
