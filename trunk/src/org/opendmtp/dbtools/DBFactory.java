...
public class DBFactory
{
    ...
    public static DBFactory getFactoryByName(String tableName)
    {
        ...
        for (Iterator i = factoryList.iterator(); i.hasNext();) {
            ...
            if (fact.getTableName().equals(tableName)) { return fact; }
        }
        return null;
    }
    ...
    public DBFactory(String tableName, DBField field[], int keyType, Class rcdClass, Class keyClass) 
    {
        ...
        for (int i = 0; i < this.field.length; i++) {
            ...
            if (this.field[i].isPriKey()) { pk.add(this.field[i]); }
            if (this.field[i].isAltKey()) { ak.add(this.field[i]); }
        }
    ...
    protected DBField[] getTableColumns()
        throws DBException
    {
    ...
    { // map old name to new name
        if (this.getTableName().equals("AssetEvent") && fn.equals("alarmCode")) { return "statusCode"; }
        return fn;
    }
    ...
    public int getFieldIndex(String name)
    {
        if (name != null) {
            ...
            for (int i = 0; i < f.length; i++) {
                if (f[i].getName().equals(realName)) { return i; }
            }
        }
        return -1;
    }
    ...
    {
        switch (type) {
            case KEY_PRIMARY: return "PRIMARY KEY";
            case KEY_UNIQUE : return "UNIQUE";
            case KEY_INDEX  : return "INDEX";
    ...
    protected DBRecordKey createKey(ResultSet rs)
        throws DBException 
    {
        ...
        throws DBException
    {
        try {
            ...
            if (sqe.getErrorCode() == SQLERR_UNKNOWN_DATABASE) {
    ...
    public boolean validateTable(boolean inclWarn)
    {
        ...
        try {
            ...
            try {
                valConst = new MethodAction(this.getRecordClass(), null, new Class[] { ResultSet.class, ValidationLog.class });
            } catch (Throwable t) { // NoSuchMethodException, ...
    ...
    protected void _dumpTable(File toFile)
        throws DBException
    {
        ...
        try {
            ...
            if (dumpOutStream != null) { try{ dumpOutStream.close(); } catch (IOException  ioe) {} }
        }
    ...
    public void loadTable(File fromFile)
        throws DBException
    {
    ...
    protected void _loadTable(String oldFieldNames[], File fromFile)
        throws DBException
    {
        ...
        try {
            ...
            for (;;) {
                ...
                if (rowValues.length != oldFieldNames.length) {
                    Print.logError("Fields - #found != #expected: " + rowValues.length + " != " + oldFieldNames.length);
                    continue;
                }
                ...
                for (int i = 0; i < oldFieldNames.length; i++) {
                    ...
                    if (field != null) {
                        if (addedField) { setSql.append(", "); }
                        
                        String val = (i < rowValues.length)? rowValues[i] : "";
                        if (val.equals("\\N")) { val = ""; } // NULL field entry
    ...
    private String[] readSQLDumpColumns(File tableSQLFile)
    {
        ...
        try {
            ...
            for (;;) {
                ...
                if (r == null) { break; }
                if ((r.length() == 0) || !Character.isWhitespace(r.charAt(0))) { continue; }
                String cdef = r.trim();
                int p = cdef.indexOf(" ");
                String cnam = cdef.substring(0, p);
                if (!cnam.equalsIgnoreCase("PRIMARY") && 
                    !cnam.equalsIgnoreCase("KEY")       ) {
                    clist.add(cnam);
                }
            }
        } catch (IOException ioe) {
