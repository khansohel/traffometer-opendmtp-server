...
public class Event {
  ...
  public Event(Packet pkt) throws PacketParseException {
    ...
    if (this.custTemplate == null) {
      Print.logError("PayloadTemplate not found: "
          + StringTools.toHexString(this.packet.getPacketType(), 8));
  ...
  /**
   * Returns event value associated with FLD_sequence for packet counter.
   * Used for event packet acknowlegement.  Incremented on client-side.
   * @return Long integer.
   */
  ...
  /**
   * Returns byte array value for given event field.
   * @param fldName String field name.
   * @param dft Byte array.
   * @return Byte array event value.
   */
   ...
   * @param fldName String field name.
   * @param dft Byte array.
   * @param ndx Integer index for position in map.
   * @return Byte array event value.
   */
  ...
  /**
   * Returns string value for given event field.
   * @param fldName String field name.
   * @param dft String.
  ...
  /**
   * Returns long integer value for given event field.
   * @param fldName String field name.
   * @param dft Long integer.
   * @return Long integer event value.
   */
  ...
  /**
   * Returns a double value for given event field.
   * @param fldName String field name.
   * @param dft Double.
  ...
  private void _decodeEvent() throws PacketParseException {
    ...
    for (this.custFieldLen = 0; payload.isValidLength(0); this.custFieldLen++) {
      ...
      case PayloadTemplate.FIELD_SPEED: // %1u 0 to 255 kph             %2u 0.0 to 655.3 kph
        this.setEventValue(GeoEvent.FLD_speedKPH,
            hiRes ? ((double) payload.readULong(length, 0L) / 10.0) : ((double) payload.readULong(
                length, 0L)));
        break;
      case PayloadTemplate.FIELD_HEADING: // %1u 1.412 deg un.            %2u 0.00 to 360.00 deg
        this.setEventValue(GeoEvent.FLD_heading,
            hiRes ? ((double) payload.readULong(length, 0L) / 100.0) : ((double) payload.readULong(
                length, 0L) * 360.0 / 255.0));
        break;
      case PayloadTemplate.FIELD_ALTITUDE: // %2i -32767 to +32767 m       %3i -838860.7 to +838860.7 m
        this.setEventValue(GeoEvent.FLD_altitude,
            hiRes ? ((double) payload.readLong(length, 0L) / 10.0) : ((double) payload.readLong(
                length, 0L)));
        break;
      case PayloadTemplate.FIELD_DISTANCE: // %3u 0 to 16777216 km         %3u 0.0 to 1677721.6 km
        this.setEventValue(GeoEvent.FLD_distanceKM,
            hiRes ? ((double) payload.readULong(length, 0L) / 10.0) : ((double) payload.readULong(
                length, 0L)));
        break;
      ...
      case PayloadTemplate.FIELD_ELAPSED_TIME: // %3u 0 to 16777216 sec        %4u 0.000 to 4294967.295 sec
        this.setEventValue(GeoEvent.FLD_elapsedTime, hiRes ? (payload.readULong(length, 0L))
      ...
      case PayloadTemplate.FIELD_TOP_SPEED: // %1u 0 to 255 kph             %2u 0.0 to 655.3 kph
        this.setEventValue(GeoEvent.FLD_topSpeedKPH, hiRes ? ((double) payload
            .readULong(length, 0L) / 10.0) : ((double) payload.readULong(length, 0L)), ndx);
        break;
      ...
      case PayloadTemplate.FIELD_GPS_HORZ_ACCURACY: // %1u 0 to 255 m               %2u 0.0 to 6553.5 m
        this.setEventValue(GeoEvent.FLD_gpsHorzAccuracy, hiRes ? ((double) payload.readULong(
            length, 0L) / 10.0) : ((double) payload.readULong(length, 0L)));
        break;
      case PayloadTemplate.FIELD_GPS_VERT_ACCURACY: // %1u 0 to 255 m               %2u 0.0 to 6553.5 m
        this.setEventValue(GeoEvent.FLD_gpsVertAccuracy, hiRes ? ((double) payload.readULong(
            length, 0L) / 10.0) : ((double) payload.readULong(length, 0L)));
        break;
      ...
      case PayloadTemplate.FIELD_GPS_MAG_VARIATION: // %2i -180.00 to 180.00 deg
        this.setEventValue(GeoEvent.FLD_gpsMagVariation,
            (double) payload.readLong(length, 0L) / 100.0);
        break;
      ...
      case PayloadTemplate.FIELD_GPS_GEOID_HEIGHT: // %1i -128 to +127 m           %2i -3276.7 to +3276.7 m
        this.setEventValue(GeoEvent.FLD_gpsGeoidHeight, hiRes ? ((double) payload.readLong(length,
            0L) / 10.0) : ((double) payload.readLong(length, 0L)));
        break;
      ...
      default:
        ...
        throw new PacketParseException(ServerErrors.NAK_FORMAT_DEFINITION_INVALID, this.packet,
            errData); // formatType, fieldIndex
      }
    }

    /* set status code if not specified in packet */
    if (!hasStatusCode) {
      this.setEventValue(GeoEvent.FLD_statusCode, hasGeoPoint ? StatusCodes.STATUS_LOCATION
          : StatusCodes.STATUS_NONE);
    }
  ...
  /**
   * Returns a string representation of all field value pairs.
   * @return a string representation of all field value pairs.
   */
  public String toString() {
    ...
    for (int i = 0; i < this.custFieldLen; i++) {
      ...
      case PayloadTemplate.FIELD_SEQUENCE: // %1u 0 to 255
        ...
        sb.append("0x");
        sb
            .append(StringTools.toHexString(this.getLongValue(GeoEvent.FLD_sequence, 0L),
                length * 8));
        break;
      ...
      case PayloadTemplate.FIELD_INPUT_STATE: // %4u 0x00000000 to 0xFFFFFFFF
        ...
        sb.append("0x");
        sb.append(StringTools.toHexString(this.getLongValue(GeoEvent.FLD_inputState, 0L),
            length * 8));
        break;
      case PayloadTemplate.FIELD_OUTPUT_ID: // %4u 0x00000000 to 0xFFFFFFFF
        sb.append("  Output ID    : ");
        sb.append("0x");
        sb
            .append(StringTools.toHexString(this.getLongValue(GeoEvent.FLD_outputID, 0L),
                length * 8));
        break;
      case PayloadTemplate.FIELD_OUTPUT_STATE: // %4u 0x00000000 to 0xFFFFFFFF
        sb.append("  Output State : ");
        sb.append("0x");
        sb.append(StringTools.toHexString(this.getLongValue(GeoEvent.FLD_outputState, 0L),
            length * 8));
        break;
      case PayloadTemplate.FIELD_ELAPSED_TIME: // %3u 0 to 16777216 sec        %4u 0.000 to 4294967.295 sec
      ...
      case PayloadTemplate.FIELD_GEOFENCE_ID: // %4u 0x00000000 to 0xFFFFFFFF
        ...
        sb.append("0x");
        sb.append(StringTools.toHexString(this.getLongValue(GeoEvent.FLD_geofenceID, 0L),
            length * 8));
        break;
      case PayloadTemplate.FIELD_TOP_SPEED: // %1u 0 to 255 kph             %2u 0.0 to 655.3 kph
        sb.append("  Top Speed    : ");
